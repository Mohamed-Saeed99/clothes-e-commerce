...
fetch('products.json')
    ...
    .then(data => {
        const productsContainers = document.getElementsByClassName('products-container');
        ...
        const productsContainers4 = document.getElementsByClassName('products-container-4'); // Add this line
    
        // Append products with IDs 1, 2, 3
        Array.from(productsContainers).forEach(container => {
            data.filter(product => [1, 2, 3].includes(product.id)).forEach(product => {
        ...
        Array.from(productsContainers2).forEach(container => {
            data.filter(product => [4, 5, 6].includes(product.id)).forEach(product => {
                const productElement = createProductElement(product);
        ...
        Array.from(productsContainers3).forEach(container => {
            data.filter(product => [7, 8, 9].includes(product.id)).forEach(product => {
                const productElement = createProductElement(product);
        ...
        // Append products with IDs 10, 11, 12
        Array.from(productsContainers4).forEach(container => {
            data.filter(product => [10, 11, 12].includes(product.id)).forEach(product => {
                const productElement = createProductElement(product);
                productElement.classList.add('different-class-name-3'); // Add this line
                container.appendChild(productElement);
            });
        });
    ...
    function createProductElement(product) {
        ...
        viewDetailsButton.addEventListener('click', function() {
            ...
            window.location.href = 'http://127.0.0.1:5501/product-details.html'; // Change the URL to the page where you want to display the details
        });
